...
ax.set(title = 'Bagging (ANOVA) Performance vs Number of Estimators K',
        ylabel='Performance (%)',xlabel='Number of Estimators K', ylim=[50, 100])
plt.legend(loc='lower right')
plt.grid(visible = True)
plt.title('Bagging (ANOVA) Performance vs Number of Estimators K', weight = 'bold')
plt.show()

# 3. Fit Bagging model with KNN for K = 8 and get accuracy score
start_time = time.time()
Y_pred_BAG, bag_clf = dt.Bagging_Classifier(X_train, Y_train, X_test, 8)
elapsed_time = time.time() - start_time
print(f"\nElapsed time to classify the data using Bagging (ANOVA) Classifier for K = 8: {elapsed_time/60:.2f} minutes")
...
print('\nBagging (ANOVA) Accuracy Score on Test data: {}%'.format(BAG_accuracy))
print('\nBagging (ANOVA) Precision Score on Test data: {}%'.format(BAG_precision))
print('\nBagging (ANOVA) Recall Score on Test data: {}%'.format(BAG_recall))

# 5. Plot non-normalized confusion matrix
titles_options = [
    ("Bagging (ANOVA) Confusion matrix", None),
    #("Bagging (ANOVA) Normalized confusion matrix", "true"),
]
...
ax.set(title = 'Boosting (ANOVA) Performance vs Number of Estimators K',
        ylabel='Performance (%)',xlabel='Number of Estimators K', ylim=[50, 110])
plt.legend(loc='lower right')
plt.grid(visible = True)
plt.title('Boosting (ANOVA) Performance vs Number of Estimators K', weight = 'bold')
plt.show()
...
elapsed_time = time.time() - start_time
print(f"\nElapsed time to classify the data using Boosting (ANOVA) Classifier for K = 4: {elapsed_time/60:.2f} minutes")
...
print('\nBoosting (ANOVA) Accuracy Score on Test data: {}%'.format(BOOST_accuracy))
print('\nBoosting (ANOVA) Precision Score on Test data: {}%'.format(BOOST_precision))
print('\nBoosting (ANOVA) Recall Score on Test data: {}%'.format(BOOST_recall))

# 5. Plot non-normalized confusion matrix
titles_options = [
    ("Boosting (ANOVA) Confusion matrix", None),
    #("Boosting (ANOVA) Normalized confusion matrix", "true"),
]
for title, normalize in titles_options:
    disp = ConfusionMatrixDisplay.from_estimator(
        boost_clf,
