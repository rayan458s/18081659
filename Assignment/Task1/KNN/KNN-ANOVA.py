...
ax.set(title = 'KNN (ANOVA) Performance vs Number of Estimators K',
        ylabel='Performance (%)',xlabel='Number of Estimators K', ylim=[50, 110])
plt.legend(loc='lower right')
plt.grid(visible = True)
plt.title('KNN (ANOVA) Performance vs Number of Estimators K', weight = 'bold')
plt.show()

# 3. Fit KNN model for K = 8 and get accuracy score
start_time = time.time()
Y_pred_KNN, KNN_clf = dt.KNN_Classifier(X_train, Y_train, X_test, 8)
elapsed_time = time.time() - start_time
print(f"\nElapsed time to classify the data using KNN (ANOVA) Classifier for K = 8: {elapsed_time/60:.2f} minutes")
...
print('\nKNN (ANOVA) Accuracy Score on Test data: {}%'.format(KNN_accuracy))
print('\nKNN (ANOVA) Precision Score on Test data: {}%'.format(KNN_precision))
print('\nKNN (ANOVA) Recall Score on Test data: {}%'.format(KNN_recall))

# 5. Plot non-normalized confusion matrix
titles_options = [
    ("KNN (ANOVA) Confusion matrix", None),
    #("KNN (ANOVA) Normalized confusion matrix", "true"),
]
